...
Seashell.prototype.type = "seashell";
...
function Lava(pos, ch) {
  ...
  if (ch == "=") {
    ...
    Lava.prototype.type = "projectile";
  } else if (ch == "|") {
    // Vertical lava
...
var wobbleSpeed = 7, wobbleDist = 0.04;

Seashell.prototype.act = function(step) {
  this.wobble += step * wobbleSpeed;
  var wobblePos = Math.sin(this.wobble) * wobbleDist;
  this.pos = this.basePos.plus(new Vector(0, wobblePos));
};
...
Level.prototype.playerTouched = function(type, actor) {
  ...
  if (type == "lava" || type == "projectile" && this.status == null) {
    this.status = "lost";
    this.finishDelay = 1;
  } else if (type == "coin" || type == "seashell") {
    this.actors = this.actors.filter(function(other) {
      return other != actor;
    });
    // If there aren't any coins left, player wins
    if (!this.actors.some(function(actor) {
           return actor.type == "coin" || actor.type == "seashell";
         })) {
      this.status = "won";
      this.finishDelay = 1;
    }
